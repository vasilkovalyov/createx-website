...
import {
  ...
  FRAGMENT_PROJECT,
  FRAGMENT_POST,
} from 'libs/cms/fragments'

class QueryAjax {
  private _url = 'http://localhost:1337/graphql'
  private graphQLClient = new GraphQLClient(this._url)

  private async getData(key, value, query) {
  ...
  public getPageSinglePost(page: string) {
    const query = gql`
    ...
    return this.getData('page', page, query)
  }

  public getPageSingleService(page: string) {
    const query = gql`
  ...
  public getRelatedProjects(category: string) {
    const query = gql`
      ...
      ${FRAGMENT_PROJECT}
    `
    return this.getData('category', category, query)
  }

  public getPageSingleProject(page: string) {
    const query = gql`
  ...
  public getPageData(page: string) {
    ...
    return this.getData('page', pageData, query)
  }
}

const instanse = new QueryAjax()
