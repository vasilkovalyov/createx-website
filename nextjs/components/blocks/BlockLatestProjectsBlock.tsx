...
export default function BlockOurProjectsBlock() {
  ...
  const data = page?.pages.data[0].attributes.Body?.filter((item) => {
    if (item.BlockType === Block.BlockLatestProjects) {
      return item.BlockType === Block.BlockLatestProjects
    }
  })[0]

  if (!data) return
  if (!page?.latestProjects.data || !page?.latestProjects.data.length) return null

  let projects: IProject[] | []

  if (page.latestProjects.data !== null) {
    projects = page.latestProjects.data.map((item) => {
